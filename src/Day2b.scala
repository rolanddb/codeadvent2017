/**
  * Created by roland on 07/12/2017.
  */
object Day2b extends App {

  def func(list: List[List[Integer]]) = {

    def findDivision(row: List[Integer]): Int = {
      ...
      result(0) // the problem statement says there should be exactly one such combination
    }

    list.map(findDivision(_)).sum
  ...
  val challenge: List[List[Integer]] = rows.map(row => row.split('\t').toList.map(Integer.valueOf(_)))
  println(challenge)
